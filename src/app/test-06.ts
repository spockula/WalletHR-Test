...
@Component({
    ...
    template : `<div style="padding: 20%;"> 
                    <h2>User Review:</h2>
                    <textarea class="textfield" placeholder="Write your Review" [value]="review_input" [(ngModel)]="review_input" (keyup)="changeReviewContent(review_input)"></textarea>
                    <br/><br/>
                    <h3>Output:</h3>
...
At https://wallethub.com <b>bolded text</b>`
    ...
    ngOnInit() {
        this.review_input = this.review_input.replace(this.urlRegex, (url: string) =>  `<a href="${url}" target="_blank">${url}</a>`)
        this.review_content = this.review_input;
    }

    changeReviewContent(review_content: string) { 
        review_content = review_content.replace(this.urlRegex, (url: string) =>  `<a href="${url}" target="_blank">${url}</a>`)
